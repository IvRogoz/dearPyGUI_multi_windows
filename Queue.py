...
def gui_A(tx):                                           
    ...
    while dpg.is_dearpygui_running():
        if data_sending:
            ...
            tx.put(waves)  # Send list of three values
            t += dt  # Increment time
...
def gui_B(rx):                                            
    ...
    while dpg.is_dearpygui_running():
        while not rx.empty():
            values = rx.get_nowait()  # Receive list of three values
...
def main():
    ...
    p1 = mp.Process(target=gui_A, args=(q,))
    p2 = mp.Process(target=gui_B, args=(q,))
    p1.start(); p2.start()
